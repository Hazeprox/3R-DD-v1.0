...
function dxGrid:AddItem ( columnIndex, text, data, r, g, b )
    ...
    if __checkParams ( "AddItem", "ns", columnIndex, text ) then
        if self.i[columnIndex] then 
            local tColor = __checkRGB ( r, g, b ) and { r, g, b } or { 0, 0, 0 };
...
function dxGrid:RemoveItem ( column, itemID )
    ...
    if __checkParams ( "RemoveItem", "nn", column, itemID ) then
        if self.i[column] and self.i[column][itemID] then
            ...
            -- Reset the selected item if necessary2
    ...
    if __checkParams ( "GetItemDetails", "nn", columnID, itemID ) then
        if self.i[column] then
            if self.i[column][itemID] then
                return self.i[column][itemID].text
            end
        end
        return false
    end
        ...
        if #dxGrid.items > 0 then
            ...
            for index, data in ipairs ( dxGrid.items ) do
                ...
                if data.vis then
                    ...
                    -- Draw the column bar
                    dxDrawRectangle ( data.x, data.y, data.w, 30 % data.h, tocolor ( 0, 0, 0, 0 ), data.pg );
                    ...
                    if data.se and data.se <= data.r and data.se >= seeFrom and data.se <= seeTo then
                        ...
                        dxDrawRectangle ( data.x, ( 30 % data.h ) + y2, data.w, 25, tocolor ( 255, 255, 255, 55 ), data.pg );
                        coloddd = {255, 155, 0}
                    end
                    ...
                    local cWidth = 0
                        ...
                        for cIndex, cData in ipairs ( data.i ) do
                            ...
                            if ( ( cWidth + cData.info.width ) <= data.w ) then
                                ...
                                dxDrawText ( cData.info.title, x, data.y, cData.info.width + x, ( 30 % data.h ) + data.y, tocolor ( 255, 255, 255 ), 2, "sans", "center", "center", true, true, data.pg, true, true );
                                ...
                                if #cData > 0 then
                                    ...
                                    for iIndex = seeFrom, seeTo do
                                        ...
                                        if cData[iIndex] then
                                            ...
                                            -- Draw the item text
                                            dxDrawText ( cData[iIndex]["text"], x, y, cData.info.width + x, ( 30 % data.h ) + y, tocolor (255,255,255,255 ), 2, "sans", "center", "center", true, true, data.pg, false, true );
                                        end
                                    end
                                end
                ...
                if cursorOn then
                    ...
                    if press and #button > 6 then
                        ...
                        if dxGrid.items[cursorOn].r > dxGrid.items[cursorOn].mi then
                            ...
                            local newValue      =   math.max ( 1, button == "mouse_wheel_down" and currentValue + 1 or currentValue -1 );
